...
.fopenLoop  move.w  #0,-(sp)                    ; read only access
            ...
            move.w  (a1)+,d0                    ; d0 holds the first sample number
            mulu.w  #samInfoLen,d0              ; get offset into sample info table
            lea     sampleInfo(pc),a3           ;
            add.w   d0,a3                       ; a3 points to sample info entry

            moveq.w #2,d7                       ; d7.w holds buffer number
            lea     packedBuf(pc),a4            ;
            add.l   #unpackBuf2-packedBuf,a4    ; a4 points to position in unpack buffer
            lea     packedBuf(pc),a5            ;
            add.l   #unpackBuf2end-packedBuf,a5 ; a5 points to end of unpack buffer
            ...
            ; remaining buffer 2 samples
            moveq.w #BUF2SAMPLES-2,d6
.buf2loop:  bsr     checkExit                   ; leave early if we have a key press
            tst.l   d0                          ;
            bne     .deinit                     ;

            move.w  (a1)+,d0                    ; d0 holds the new sample number
            mulu.w  #samInfoLen,d0              ; get offset into sample info table
            lea     sampleInfo(pc),a3           ;
            add.w   d0,a3                       ; a3 points to sample info entry
            ...
            ; loaded all the recurring samples
            lea     loadedBuf2Str(pc),a0
            bsr     cconws

            ; buffer 0 and 1 samples
...
.loadedAll: lea     loadedAllStr(pc),a0
            bsr     cconws
...
loadedBuf2Str:
            dc.b    "finished loading recurring samples",13,10,0
loadedAllStr:
...
buf2Samples:dc.w    0,1,8,11,12
buf2SamplesEnd:

BUF2SAMPLES equ     (buf2SamplesEnd-buf2Samples)/2
...
unpackBuf0: ds.b    351443                  ; assembles to ~962946 bytes which runs on a 1MB TOS1.62 machine in the auto folder
unpackBuf0end:

unpackBuf1: ds.b    unpackBuf0end-unpackBuf0
unpackBuf1end:

unpackBuf2: ds.b    SAMPLE0LEN
            ds.b    SAMPLE1LEN
            ds.b    SAMPLE8LEN
            ds.b    SAMPLE11LEN
            ds.b    SAMPLE12LEN
